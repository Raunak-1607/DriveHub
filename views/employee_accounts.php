            ...
            <table id="employeeTable">
                ...
                if (!empty($employees)) {
                    foreach ($employees as $e) {
                        ...
                        echo "<td><span class='" . $badgeClass . "'>&#9679; " . htmlspecialchars($e['account_status']) . "</span></td>";

                        echo "<td><div class='actions-cell'>";
                        echo "<a class='btn-view action-btn' href='edit_employee.php?id=" . urlencode($e['id']) . "'>Edit</a>";
                        if ($isActive) {
                            echo "<a class='btn-reject action-btn' href='../controllers/adminController.php?action=deactivate_employee&id=" . urlencode($e['id']) . "'>Deactivate</a>";
                        } else {
                            echo "<a class='btn-approve action-btn' href='../controllers/adminController.php?action=activate_employee&id=" . urlencode($e['id']) . "'>Activate</a>";
                        }
                        echo "<a class='btn-reject action-btn' href='../controllers/adminController.php?action=delete_employee&id=" . urlencode($e['id']) . "' onclick=\"return confirm('Delete this employee?');\">Delete</a>";
                        echo "</div></td>";
                        echo "</tr>";
                    }
                } else {
